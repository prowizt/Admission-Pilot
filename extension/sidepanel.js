...
function addMessage(text, isUser = false, saveToStorage = true, logId = null, initialFeedback = null, latencyMs = null) {
  ...
  innerDiv.innerHTML = isUser ? formatChatText(text) : formatChatText(text);
  ...
  if (isUser) {
    ...
    const handleFeedback = async (btn, type) => {
      try {
        ...
        await fetch(`http://210.93.0.229:8080/api/logs/audit/${logId}/feedback`, {
          method: 'PUT',
...
async function sendMessage() {
  ...
  let endpoint = 'http://210.93.0.229:8080/api/chat';
  let bodyData;
  let headers = {
  ...
  if (attachedExcelFile && isAutofillIntent) {
    endpoint = 'http://210.93.0.229:8080/api/chat-excel-autofill';
...
async function processFile(file) {
  ...
  try {
    const response = await fetch('http://210.93.0.229:8080/api/parse-file', {
      method: 'POST',
      body: formData
    });
...
function checkServerStatus() {
  fetch('http://210.93.0.229:8080/api/health')
    .then(res => {
      if (res.ok) {
