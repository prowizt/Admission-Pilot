...
> [!IMPORTANT]
> **[행동 대원칙: 선 계획 수립 및 승인 대기]**
> 사용자의 어떠한 코드 수정, 추가, 디버깅 요청이든 임의로 바로 작업을 실행하지 마십시오.
> 무조건 작업 전에 **`implementation_plan.md` 구현 계획서**를 아티팩트 디렉토리에 가장 먼저 작성하고, 사용자의 명시적인 검토 및 승인 지시가 있을 때까지 작업을 멈추고 대기해야 합니다.
...
6. **선 계획 수립 및 승인 대기 (Critical Workflow):**
   * 어떠한 기능 구현이나 버그 수정 요청이든 작업을 즉시 실행하지 마라.
   * 무조건 `implementation_plan.md` 파일에 구현 플랜을 구조화하여 업데이트한 후, 사용자로부터 "승인" 또는 "진행" 지시가 있기 전까지는 대기 상태를 유지해야 한다.
